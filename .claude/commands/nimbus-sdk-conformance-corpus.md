...
The corpus is how a claim in a normative document becomes something two languages are
...
| Corpus | Normative document | TypeScript runner | Python runner |
|---|---|---|---|
| `conformance/v1/negotiation/` | `negotiation/v1/contract-version.md` | `scripts/negotiation-guard.test.ts` | `tests/test_negotiation_corpus.py` |
| `conformance/v1/framing/` | `wire/v1/framing.md` | `scripts/framing-guard.test.ts` (+ `framing-node.mjs` under Node LTS) | `tests/test_framing_corpus.py` |
| `conformance/v1/diagnostics/` | `diagnostics/v1/diagnostics.md` | `scripts/diagnostics-guard.test.ts` | `tests/test_diagnostics_corpus.py` |
| `conformance/v1/predicates/` | `predicates/v1/README.md` | `scripts/predicates-guard.test.ts` | — TypeScript only |
| `conformance/v1/sandbox/` | `probe/v1/` | `scripts/sandbox-guard.test.ts` (+ `probe-runtime.test.ts`) | — TypeScript only |
| `conformance/v1/index.json` → `manifest/`, `item/` | `schemas/v1/` + `rules/v1/` | `scripts/schema-guard.test.ts`, `scripts/rules-guard.test.ts` | — TypeScript only |

All TypeScript guard paths are relative to `sdks/typescript/`; all Python test paths to
`sdks/python/`. Guards resolve the spec tree through `repoRoot` from
...
4. **Reinstall the Python package** before running `pytest` — see the trap below.
5. **Run both suites.** `bun run test` from the repository root; then from
   `sdks/python/`: `python -m pip install -e . && python -m pytest -q`.
...
|---|---|---|
| `negotiation` | `^§[0-9]+$` | `"§6"` |
| `diagnostics` | `^§[0-9]+(\.[0-9]+)*$` — wider on purpose, because `diagnostics.md` has a real subsection (§5.1) a chapter-only pattern cannot name | `"§5.1"` |
...
- `test_negotiation_corpus_loads` — `assert len(cases) == 37`, plus a `kind` set equality
  over `{"declaration", "hello", "negotiate"}`.
- `test_framing_corpus_loads` — `assert len(cases) == 25`.
...
- **`docs/ROADMAP.md`** — only if a box's claim (a corpus size, a "both bindings execute
