...
- **SDK owners** — for each official language binding, the people responsible for
  ...
  - **TypeScript** (`@nimbus-dev/sdk`) — Asaf Golombek
    ...
    Python's ten and Go's ten are each executed **in full**. The **partial** claim
    recorded here while [RFC-0020](./rfcs/0020-manifest-signing.md)'s S2 shipped — Python
    running `manifest-signature`'s `base64url` and `thumbprint` kinds and deferring the
    three that need Ed25519, which CPython has no stdlib primitive for — is retired: S3
    gave `nimbus_sdk.signing` a from-scratch RFC 8032 implementation, and no binding now
    defers a case in any corpus it claims. See
    [`docs/conformance-coverage.md`](./conformance-coverage.md) for the
    executed-of-published case counts rather than restating generated numbers here.
    The distinction that episode exposed is kept even though nothing currently turns on
    it: claiming a corpus and executing every case of it are two statements, criterion 1
    as [RFC-0013](./rfcs/0013-go-sdk-official.md) defines it — *every published corpus
    whose surface the binding publishes* — is satisfiable at case granularity as well as
    corpus granularity, and the generated page is where either kind of gap would be
    visible. "Executes N" happens to mean "executes N in full" again; it is not
    guaranteed to.
