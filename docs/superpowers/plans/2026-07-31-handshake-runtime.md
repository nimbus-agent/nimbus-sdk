...
- **Four CI gates fire here, not two, and they have different triggers.** A changed *export* trips one: regenerate `docs/api-surface.md` with `bun run api:surface` (`scripts/api-surface.test.ts`). A *new module* in the published surface trips two more, both keyed on the module rather than the export: claim it in a `docs/modules/*.md` page's `<!-- covers: -->` comment (`scripts/docs-coverage.test.ts`) — `docs/modules/ipc.md` currently reads `<!-- covers: ipc/ndjson-line-reader -->`, so `ipc/handshake` goes there — and add an entry to `scripts/smoke-calls.mjs`, which `scripts/smoke-calls.test.ts` executes against the built `dist/`. And a fenced `ts` block added to `docs/modules/`, `docs/README.md`, or the package `README.md` trips a fourth: `scripts/docs-snippets.test.ts` typechecks all of them against `dist/`. Tasks 1 and 2 each rediscovered one of these the hard way.
