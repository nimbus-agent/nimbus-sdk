...
- `nimbus_sdk.ipc` (`sdks/python/src/nimbus_sdk/ipc/`) — `hello.py` (`encode_hello`,
  `parse_hello`) and `ndjson.py` (`NdjsonLineReader`, `FlushResult`,
  `FrameTooLongError`).
...
- A new/changed **TypeScript** export requires regenerating `docs/api-surface.md`
  **and** a `docs/modules/*.md` page claiming its module in `<!-- covers: -->` — both
  are CI gates (`sdks/typescript/scripts/api-surface.test.ts`,
  `sdks/typescript/scripts/docs-coverage.test.ts`). Both read TypeScript only; there is
  no equivalent gate for the Python surface. Note `api-surface.md` also lists `private`
  members, so an internal-only change to a published class still fails the gate until
  you re-run `bun run api:surface`.
