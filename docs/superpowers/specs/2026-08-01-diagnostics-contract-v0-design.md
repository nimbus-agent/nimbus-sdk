...
divergence between the bindings**. CLAUDE.md's claim that there are exactly two deliberate
differences stays true and does not need amending on that point.
...
same corpus under mypy strict and ruff.
...
- **An injected clock / id seam** (`createEmitter` with `now` and `newCorrelationId`) —
  strictly additive over the chosen design, and a minor when it lands.
