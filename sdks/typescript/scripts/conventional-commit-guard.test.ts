...
describe("repo-root anchoring (Finding A)", () => {
  ...
  //              BASE side still parses all 248 TypeScript entries, while the HEAD side
  ...
  // cwd where the original bug bit. `parseSurface` is the guard's own parser, and 248 is
  ...
  test("the head golden is readable through repo anchoring from the package root", () => {
    ...
    try {
      ...
      expect(parseSurface(readFileSync(golden, "utf8")).size).toBe(248);
    } finally {
      process.chdir(original);
    }
  });
